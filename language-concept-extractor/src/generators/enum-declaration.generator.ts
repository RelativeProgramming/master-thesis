...
export class EnumDeclarationGenerator extends Generator {
    ...
    async run(neo4jSession: Session, concepts: Map<string, LCEConcept[]>, connectionIndex: ConnectionIndex): Promise<void> {
        ...
        for(let enumDecl of enumDecls) {
            ...
            const enumNodeId = Utils.getNodeIdFromQueryResult(await neo4jSession.run(
                `
            ...
            for(let i = 0; i < enumDecl.members.length; i++) {
                ...
                const enumMemberNodeId = Utils.getNodeIdFromQueryResult(await neo4jSession.run(
                    `
                ...
                if(enumMemberDecl.init) {
                    await createValueNode(
                        enumMemberDecl.init,
                        neo4jSession,
                        connectionIndex,
                        enumMemberNodeId,
                        {name: ":INITIALIZED_WITH", props: {}}
                    );
                }
                

                // link enum member to enum declaration
                connectionIndex.connectionsToCreate.push([enumNodeId, enumMemberNodeId, {name: ":DECLARES", props: {index: i}}]);
            }

 ...
 */
 export async function createValueNode(
    ...
    if(value instanceof LCEValueNull) {
        ...
        for(let [key, member] of value.members.entries()) {
            await createValueNode(member, neo4jSession, connectionIndex, valueNodeId, {name: ":HAS_MEMBER", props:{name: key}});
        }
        ...
        for(let i = 0; i < value.items.length; i++) {
            ...
            await createValueNode(item, neo4jSession, connectionIndex, valueNodeId, {name: ":CONTAINS", props:{index: i}});
        }
        ...
        for(let i = 0; i < value.args.length; i++) {
            ...
            await createValueNode(arg, neo4jSession, connectionIndex, valueNodeId, {name: ":HAS_ARGUMENT", props:{index: i}});
        }

    ...
    public traverseChildren(processingContext: ProcessingContext, processors: ProcessorMap): ConceptMap {
        ...
        if(node.type === AST_NODE_TYPES.MethodDefinition || node.type === AST_NODE_TYPES.TSMethodSignature) {
            ...
            if(node.typeParameters)
                runTraverserForNodes(node.typeParameters.params, {parentPropName: MethodTraverser.TYPE_PARAMETERS_PROP}, processingContext, processors, conceptMaps);
            
            runTraverserForNode(node.key, {parentPropName: MethodTraverser.KEY_PROP}, processingContext, processors, conceptMaps);
            
            if(node.type === AST_NODE_TYPES.MethodDefinition) {
                runTraverserForNodes(node.value.params, {parentPropName: MethodTraverser.PARAMETERS_PROP}, processingContext, processors, conceptMaps);
                if(node.decorators)
                    runTraverserForNodes(node.decorators, {parentPropName: MethodTraverser.DECORATORS_PROP}, processingContext, processors, conceptMaps);
                if(node.value.body)
                    runTraverserForNodes(node.value.body.body, {parentPropName: MethodTraverser.BODY_PROP}, processingContext, processors, conceptMaps);
            } else {
...
export class ParameterPropertyTraverser extends Traverser {
    ...
    public traverseChildren(processingContext: ProcessingContext, processors: ProcessorMap): ConceptMap {
        ...
        if(node.type === AST_NODE_TYPES.TSParameterProperty && node.decorators) {
            return runTraverserForNodes(node.decorators, {parentPropName: ParameterPropertyTraverser.DECORATORS_PROP}, processingContext, processors) ?? new Map();
        }

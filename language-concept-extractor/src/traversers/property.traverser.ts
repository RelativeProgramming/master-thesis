...
export class PropertyTraverser extends Traverser {
    ...
    public traverseChildren(processingContext: ProcessingContext, processors: ProcessorMap): ConceptMap {
        ...
        if(node.type === AST_NODE_TYPES.PropertyDefinition || node.type === AST_NODE_TYPES.Property || node.type === AST_NODE_TYPES.TSPropertySignature) {
            ...
            if(node.type === AST_NODE_TYPES.PropertyDefinition || node.type === AST_NODE_TYPES.Property) {
                if(node.value)
                    runTraverserForNode(node.value, {parentPropName: PropertyTraverser.INITIALIZER_PROP}, processingContext, processors, conceptMaps);
                if(node.type === AST_NODE_TYPES.PropertyDefinition && node.decorators) 
                    runTraverserForNodes(node.decorators, {parentPropName: PropertyTraverser.DECORATORS_PROP}, processingContext, processors, conceptMaps)
            } else {
                if(node.initializer) {
                    return runTraverserForNode(node.initializer, {parentPropName: PropertyTraverser.INITIALIZER_PROP}, processingContext, processors) ?? new Map();
                }
            }
        }

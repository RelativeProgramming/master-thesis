    ...
    public traverseChildren(processingContext: ProcessingContext, processors: ProcessorMap): ConceptMap {
        ...
        if(node.type === AST_NODE_TYPES.TSTypeAliasDeclaration) {
            if(node.typeParameters) {
                runTraverserForNodes(node.typeParameters.params, {parentPropName: TypeAliasDeclarationTraverser.TYPE_PARAMETERS_PROP}, processingContext, processors, conceptMaps);
            }
        }

    ...
    public traverseChildren(processingContext: ProcessingContext, processors: ProcessorMap): ConceptMap {
        ...
        if(node.type === AST_NODE_TYPES.ClassDeclaration || node.type === AST_NODE_TYPES.ClassExpression) {
            ...
            if(node.typeParameters) {
                runTraverserForNodes(node.typeParameters.params, {parentPropName: ClassTraverser.TYPE_PARAMETERS_PROP}, processingContext, processors, conceptMaps);
            }
            if(node.superClass) {
            ...
            if(node.superTypeParameters) {
                runTraverserForNodes(node.superTypeParameters.params, {parentPropName: ClassTraverser.EXTENDS_TYPE_PARAMETERS_PROP}, processingContext, processors, conceptMaps);
            }

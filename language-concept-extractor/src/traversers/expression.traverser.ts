...
export class ArrayExpressionTraverser extends Traverser {
    ...
    public traverseChildren(processingContext: ProcessingContext, processors: ProcessorMap): ConceptMap {
        ...
        if(node.type === AST_NODE_TYPES.ArrayExpression) {
            runTraverserForNodes(node.elements, {parentPropName: ArrayExpressionTraverser.ELEMENTS_PROP}, processingContext, processors, conceptMaps);
        }
...
export class ArrayPatternTraverser extends Traverser {
    ...
    public traverseChildren(processingContext: ProcessingContext, processors: ProcessorMap): ConceptMap {
        ...
        if(node.type === AST_NODE_TYPES.ArrayPattern) {
            if(node.decorators)
                runTraverserForNodes(node.decorators, {parentPropName: ArrayPatternTraverser.DECORATORS_PROP }, processingContext, processors, conceptMaps);
    ...
    public traverseChildren(processingContext: ProcessingContext, processors: ProcessorMap): ConceptMap {
        ...
        if(node.type === AST_NODE_TYPES.ArrowFunctionExpression) {
            if(node.typeParameters)
                runTraverserForNodes(node.typeParameters.params, {parentPropName: ArrowFunctionExpressionTraverser.TYPE_PARAMETERS_PROP }, processingContext, processors, conceptMaps);
            runTraverserForNodes(node.params, {parentPropName: ArrowFunctionExpressionTraverser.PARAMETERS_PROP }, processingContext, processors, conceptMaps);
            runTraverserForNode(node.body, {parentPropName: ArrowFunctionExpressionTraverser.BODY_PROP }, processingContext, processors, conceptMaps);
        }
...
export class AwaitExpressionTraverser extends Traverser {
    ...
    public traverseChildren(processingContext: ProcessingContext, processors: ProcessorMap): ConceptMap {
        ...
        if(node.type === AST_NODE_TYPES.AwaitExpression) {
            runTraverserForNode(node.argument, {parentPropName: AwaitExpressionTraverser.ARGUMENT_PROP }, processingContext, processors, conceptMaps);
        }
...
export class CallExpressionTraverser extends Traverser {
    ...
    public traverseChildren(processingContext: ProcessingContext, processors: ProcessorMap): ConceptMap {
        ...
        if(node.type === AST_NODE_TYPES.CallExpression) {
            if(node.typeParameters)
                runTraverserForNodes(node.typeParameters.params, {parentPropName: CallExpressionTraverser.TYPE_PARAMETERS_PROP }, processingContext, processors, conceptMaps);
            runTraverserForNodes(node.arguments, {parentPropName: CallExpressionTraverser.ARGUMENTS_PROP }, processingContext, processors, conceptMaps);    
...
export class ChainExpressionTraverser extends Traverser {
    ...
    public traverseChildren(processingContext: ProcessingContext, processors: ProcessorMap): ConceptMap {
        ...
        if(node.type === AST_NODE_TYPES.ChainExpression) {
            runTraverserForNode(node.expression, {parentPropName: ChainExpressionTraverser.EXPRESSION_PROP }, processingContext, processors, conceptMaps);
        }
...
export class ConditionalExpressionTraverser extends Traverser {
    ...
    public traverseChildren(processingContext: ProcessingContext, processors: ProcessorMap): ConceptMap {
        ...
        if(node.type === AST_NODE_TYPES.ConditionalExpression) {
            ...
            runTraverserForNode(node.consequent, {parentPropName: ConditionalExpressionTraverser.CONSEQUENT_PROP }, processingContext, processors, conceptMaps);
            runTraverserForNode(node.alternate, {parentPropName: ConditionalExpressionTraverser.ALTERNATE_PROP }, processingContext, processors, conceptMaps);
        }
...
export class ImportExpressionTraverser extends Traverser {
    ...
    public traverseChildren(processingContext: ProcessingContext, processors: ProcessorMap): ConceptMap {
        ...
        if(node.type === AST_NODE_TYPES.ImportExpression) {
            ...
            if(node.attributes)
                runTraverserForNode(node.attributes, {parentPropName: ImportExpressionTraverser.ATTRIBUTES_PROP }, processingContext, processors, conceptMaps);
        }
...
export class MemberExpressionTraverser extends Traverser {
    ...
    public traverseChildren(processingContext: ProcessingContext, processors: ProcessorMap): ConceptMap {
        ...
        if(node.type === AST_NODE_TYPES.MemberExpression) {
            ...
            runTraverserForNode(node.property, {parentPropName: MemberExpressionTraverser.PROPERTY_PROP }, processingContext, processors, conceptMaps);
        }
...
export class NewExpressionTraverser extends Traverser {
    ...
    public traverseChildren(processingContext: ProcessingContext, processors: ProcessorMap): ConceptMap {
        ...
        if(node.type === AST_NODE_TYPES.NewExpression) {
            if(node.typeParameters)
                runTraverserForNodes(node.typeParameters.params, {parentPropName: NewExpressionTraverser.TYPE_PARAMETERS_PROP }, processingContext, processors, conceptMaps);
            runTraverserForNodes(node.arguments, {parentPropName: NewExpressionTraverser.ARGUMENTS_PROP }, processingContext, processors, conceptMaps);
...
export class ObjectExpressionTraverser extends Traverser {
    ...
    public traverseChildren(processingContext: ProcessingContext, processors: ProcessorMap): ConceptMap {
        ...
        if(node.type === AST_NODE_TYPES.ObjectExpression) {
            runTraverserForNodes(node.properties, {parentPropName: ObjectExpressionTraverser.PROPERTIES_PROP }, processingContext, processors, conceptMaps);
        }
...
export class ObjectPatternTraverser extends Traverser {
    ...
    public traverseChildren(processingContext: ProcessingContext, processors: ProcessorMap): ConceptMap {
        ...
        if(node.type === AST_NODE_TYPES.ObjectPattern) {
            if(node.decorators)
                runTraverserForNodes(node.decorators, {parentPropName: ObjectPatternTraverser.DECORATORS_PROP }, processingContext, processors, conceptMaps);
            runTraverserForNodes(node.properties, {parentPropName: ObjectPatternTraverser.PROPERTIES_PROP }, processingContext, processors, conceptMaps);
        }
...
export class SequenceExpressionTraverser extends Traverser {
    ...
    public traverseChildren(processingContext: ProcessingContext, processors: ProcessorMap): ConceptMap {
        ...
        if(node.type === AST_NODE_TYPES.SequenceExpression) {
            runTraverserForNodes(node.expressions, {parentPropName: SequenceExpressionTraverser.EXPRESSIONS_PROP }, processingContext, processors, conceptMaps);
        }
...
export class TaggedTemplateExpressionTraverser extends Traverser {
    ...
    public traverseChildren(processingContext: ProcessingContext, processors: ProcessorMap): ConceptMap {
        ...
        if(node.type === AST_NODE_TYPES.TaggedTemplateExpression) {
            if(node.typeParameters)
                runTraverserForNodes(node.typeParameters.params, {parentPropName: TaggedTemplateExpressionTraverser.TYPE_PARAMETER_PROP}, processingContext, processors, conceptMaps);
            ...
            runTraverserForNode(node.quasi, {parentPropName: TaggedTemplateExpressionTraverser.QUASI_PROP}, processingContext, processors, conceptMaps);
        }
...
export class TemplateLiteralTraverser extends Traverser {
    ...
    public traverseChildren(processingContext: ProcessingContext, processors: ProcessorMap): ConceptMap {
        ...
        if(node.type === AST_NODE_TYPES.TemplateLiteral) {
            ...
            runTraverserForNodes(node.expressions, {parentPropName: TemplateLiteralTraverser.EXPRESSIONS_PROP }, processingContext, processors, conceptMaps);
        }
...
export class AsExpressionTraverser extends Traverser {
    ...
    public traverseChildren(processingContext: ProcessingContext, processors: ProcessorMap): ConceptMap {
        ...
        if(node.type === AST_NODE_TYPES.TSAsExpression) {
            runTraverserForNode(node.expression, {parentPropName: AsExpressionTraverser.EXPRESSION_PROP }, processingContext, processors, conceptMaps);
        }
...
export class NonNullExpressionTraverser extends Traverser {
    ...
    public traverseChildren(processingContext: ProcessingContext, processors: ProcessorMap): ConceptMap {
        ...
        if(node.type === AST_NODE_TYPES.TSNonNullExpression) {
            if(node.expression)
                runTraverserForNode(node.expression, {parentPropName: NonNullExpressionTraverser.EXPRESSION_PROP }, processingContext, processors, conceptMaps);
        }
...
export class TypeAssertionTraverser extends Traverser {
    ...
    public traverseChildren(processingContext: ProcessingContext, processors: ProcessorMap): ConceptMap {
        ...
        if(node.type === AST_NODE_TYPES.TSTypeAssertion) {
            runTraverserForNode(node.expression, {parentPropName: TypeAssertionTraverser.EXPRESSION_PROP }, processingContext, processors, conceptMaps);
        }
...
export class UnaryExpressionTraverser extends Traverser {
    ...
    public traverseChildren(processingContext: ProcessingContext, processors: ProcessorMap): ConceptMap {
        ...
        if(node.type === AST_NODE_TYPES.UnaryExpression) {
            runTraverserForNode(node.argument, {parentPropName: UnaryExpressionTraverser.ARGUMENT_PROP }, processingContext, processors, conceptMaps);
        }
...
export class UpdateExpressionTraverser extends Traverser {
    ...
    public traverseChildren(processingContext: ProcessingContext, processors: ProcessorMap): ConceptMap {
        ...
        if(node.type === AST_NODE_TYPES.UpdateExpression) {
            runTraverserForNode(node.argument, {parentPropName: UpdateExpressionTraverser.ARGUMENT_PROP }, processingContext, processors, conceptMaps);
        }
...
export class YieldExpressionTraverser extends Traverser {
    ...
    public traverseChildren(processingContext: ProcessingContext, processors: ProcessorMap): ConceptMap {
        ...
        if(node.type === AST_NODE_TYPES.YieldExpression) {
            if(node.argument)
                runTraverserForNode(node.argument, {parentPropName: YieldExpressionTraverser.ARGUMENT_PROP }, processingContext, processors, conceptMaps);
        }

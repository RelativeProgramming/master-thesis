...
export class VariableDeclarationTraverser extends Traverser {
    ...
    public traverseChildren(processingContext: ProcessingContext, processors: ProcessorMap): ConceptMap {
        ...
        if(node.type === AST_NODE_TYPES.VariableDeclaration) {
            runTraverserForNodes(node.declarations, {parentPropName: VariableDeclarationTraverser.DECLARATIONS_PROP}, processingContext, processors, conceptMaps);
        }
    ...
    public traverseChildren(processingContext: ProcessingContext, processors: ProcessorMap): ConceptMap {
        ...
        if(node.type === AST_NODE_TYPES.VariableDeclarator) {
            if(node.id.decorators)
                runTraverserForNodes(node.id.decorators, {parentPropName: VariableDeclaratorTraverser.DECORATORS_PROP}, processingContext, processors, conceptMaps);
            if(node.init)

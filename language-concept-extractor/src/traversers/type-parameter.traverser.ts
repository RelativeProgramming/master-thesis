...
export class TypeParameterDeclarationTraverser extends Traverser {
    ...
    public processChildren(processingContext: ProcessingContext, processors: ProcessorMap): ConceptMap {
        ...
        if(node.type === AST_NODE_TYPES.TSTypeParameterDeclaration) {
            runTraverserForNodes(node.params, {parentPropName: TypeParameterDeclarationTraverser.PARAMS_PROP}, processingContext, processors, conceptMaps);
        }
...
export class TypeParameterInstantiationTraverser extends Traverser {
    ...
    public processChildren(processingContext: ProcessingContext, processors: ProcessorMap): ConceptMap {
        ...
        if(node.type === AST_NODE_TYPES.TSTypeParameterInstantiation) {
            runTraverserForNodes(node.params, {parentPropName: TypeParameterInstantiationTraverser.PARAMS_PROP}, processingContext, processors, conceptMaps);
        }
...
export class TypeParameterTraverser extends Traverser {
    ...
    public processChildren(processingContext: ProcessingContext, processors: ProcessorMap): ConceptMap {
        ...
        if(node.type === AST_NODE_TYPES.TSTypeParameter) {
            if(node.constraint)
                return runTraverserForNode(node.constraint, {parentPropName: TypeParameterTraverser.CONSTRAINT_PROP}, processingContext, processors) ?? new Map();
        }

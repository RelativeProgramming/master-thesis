...
export class PathUtils {
    ...
    static normalize(projectPath: string, path: string, originPath?: string): string {
        ...
        if(pathType === "node") {
            ...
            const relPath = pathType === "absolute" ? 
                p.relative(projectPath, path) :
                p.relative(projectPath, p.resolve(projectPath, originPath!.slice(0, originPath!.lastIndexOf("/")), path));
    ...
    static normalizeImportPath(projectPath: string, importPath: string, originPath?: string): string {
        ...
        if(pathType === "absolute") {
            ...
            return this.normalize(projectPath, this.addFileEnding(p.resolve(projectPath, originPath.slice(0, originPath.lastIndexOf("/")), importPath)));
        } else {
            return importPath;
        }
    }
